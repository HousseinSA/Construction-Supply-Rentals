...
import { Eye, Clock, CheckCircle, XCircle, User, Loader2, ArrowLeft, Building } from 'lucide-react'
...
export default function BookingTable() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      paid: { color: 'bg-blue-100 text-blue-800', icon: CheckCircle, text: t('status.paid') },
      completed: { color: 'bg-green-100 text-green-800', icon: CheckCircle, text: t('status.completed') },
      cancelled: { color: 'bg-red-100 text-red-800', icon: XCircle, text: t('status.cancelled') }
    }
                      ...
                      <tr key={booking._id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-500">
                            {booking.bookingItems.length} {t('items') || 'items'}
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className="text-sm text-gray-900">
                            ...
                            {booking.bookingItems.length > 1 && (
                              <span className="text-gray-500">
                                {' '}+{booking.bookingItems.length - 1} {t('more') || 'more'}
                              </span>
                            )}
                          </div>
                        </td>
              ...
              {paginatedBookings.map((booking) => (
                <div key={booking._id} className="p-4 hover:bg-gray-50">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="text-xs text-gray-500 ml-8" dir="ltr">
                      {booking.renterInfo[0]?.phone}
                    </div>
                  </div>
