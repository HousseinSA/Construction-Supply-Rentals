...
export default function ManageEquipmentClient() {
  ...
  console.log("all equipement data", equipment)
  useEffect(() => {
    fetchEquipment()
  }, [])
          ...
          if (item.supplierId && usersData.success) {
            const supplierId =
              typeof item.supplierId === "object"
                ? item.supplierId.$oid || item.supplierId.toString()
                : item.supplierId
            const supplier = usersData.data.find(
              (user: any) =>
                user._id === supplierId || user._id.toString() === supplierId
            )
            if (supplier) {
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast.success(
          t(
            newStatus === "approved" ? "equipmentApproved" : "equipmentRejected"
          )
        )
      }
    } catch (error) {
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-gray-50 border-b">
                      <tr>
                        <th
                          className={`px-6 py-4 ${headerAlign} text-sm font-semibold text-gray-700`}
                        >
                          {t("name")}
                        </th>
                        <th
                            ...
                            <div className="flex items-center justify-center gap-2">
                              {item.createdBy === "admin" && (
                                <button
                                  onClick={() =>
                                    router.push(
                                      `/dashboard/equipment/edit/${item._id}`
                                    )
                                  }
                              ...
                              <button
                                onClick={() =>
                                  router.push(`/equipment/${item._id}`)
                                }
                  ...
                  <div key={item._id} className="p-4 hover:bg-gray-50">
                    <div className="flex gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        {item.createdBy === "admin" && (
                          <span className="inline-block px-2 py-0.5 text-xs bg-blue-100 text-blue-700 rounded mb-1">
                            {t("createdByAdmin")}
                          </span>
                        )}
                    ...
                    <div className="flex gap-2">
                      {item.createdBy === "admin" && (
                        <button
                          onClick={() =>
                            router.push(`/dashboard/equipment/edit/${item._id}`)
                          }
                          className="flex-1 p-2 text-blue-600 bg-blue-50 rounded-lg font-medium text-sm"
                        >
                      ...
                      <button
                        ...
                        className="flex-1 p-2 text-gray-600 bg-gray-100 rounded-lg font-medium text-sm"
                      >
