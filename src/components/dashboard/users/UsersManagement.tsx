...
export default function UsersManagement() {
  ...
  const openConfirmModal = (userId: string, action: 'block' | 'unblock', userName: string) => {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (success) {
        showToast.success(t(`users.user${confirmModal.action === 'block' ? 'Blocked' : 'Unblocked'}`))
      } else {
    ...
    <div className="space-y-6">
      ...
      {confirmModal.action && (
        <ConfirmModal
          ...
          title={t(`users.confirm${confirmModal.action === "block" ? "Block" : "Unblock"}Title`)}
          message={t(`users.confirm${confirmModal.action === "block" ? "Block" : "Unblock"}Message`)}
