...
import { User, Mail, Phone, MapPin, MessageCircle, Building2 } from 'lucide-react'
...
export default function SupplierInfo({ supplier, variant = 'card', showContactButtons = true }: SupplierInfoProps) {
  const t = useTranslations('equipmentDetails')
  const tBooking = useTranslations('dashboard.bookings.details')

  if (!supplier) return null

  // Modal variant - matches renter info style
  if (variant === 'modal') {
    return (
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <span className="font-medium" dir="ltr">{formatPhoneNumber(supplier.phone)}</span>
      ...
      <div className="space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            {supplier.companyName && (
              <div className="text-sm text-gray-600">{supplier.companyName}</div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href={`mailto:${supplier.email}`} className="text-gray-700 hover:text-orange-600 transition-colors">
            {supplier.email}
          </a>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-1">
            <span className="text-gray-900 font-medium" dir="ltr">{formatPhoneNumber(supplier.phone)}</span>
