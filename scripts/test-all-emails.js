...
async function testAllEmails() {
  ...
  // Import from compiled Next.js build
  let emailLib;
  try {
    // Try to import from Next.js build first
    emailLib = await import('../.next/server/chunks/src_lib_email_index_ts.js').catch(() => 
      import('../.next/server/app/api/test-emails/route.js').catch(() => {
        throw new Error('Next.js build not found. Run: npm run build');
      })
    );
  } catch (buildError) {
    console.log('⚠️  Next.js build not found. Trying direct import...\n');
    // Fallback: use tsx to run TypeScript directly
    const { execSync } = require('child_process');
    console.log('💡 Install tsx: npm install -D tsx');
    console.log('💡 Then run: npx tsx scripts/test-all-emails.ts\n');
    throw new Error('Cannot import TypeScript files directly. Build Next.js first with: npm run build');
  }
